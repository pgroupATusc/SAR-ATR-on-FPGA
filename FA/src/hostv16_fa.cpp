...
int main(int argc, char** argv) {
    ...
    std::cout<< "reading the test data\n";
        ...
        for(int f = 0; f < f_block_len_custom; f++){
            for(int j = 0; j < VDATA_SIZE; j++){
                if(j == 0 && f == 0){
                    ...
                    boX_map[i*f_block_len_custom + f].data[j] = (datatype) fdata;
                    boX_map_bank1[i].data[j] = (datatype) fdata;
                }
                else{
                    boX_map[i*f_block_len_custom + f].data[j] = 0;
                    boX_map_bank1[i].data[j] = 0;
                }
            }
        }
    }
    ...
    std::ifstream fin2("../data/myout.bin", std::ios::binary);
    for(int i = 0; i < num_vertex; i++){
        for(int f = 0; f < f_block_len_custom; f++){
            for(int j = 0; j < VDATA_SIZE; j++){
                ...
                if(f == 0 && j ==0 ){
                    fin2.read(reinterpret_cast<char*>(&fdata), sizeof(float));
                    if(boAX_map_bank1[i].data[j] != fdata){
                        printf("result not match\n");
                        printf("%.10f, %.10f\n", (float) boAX_map_bank1[i].data[j], fdata);
                    }
                }
            }
        }
