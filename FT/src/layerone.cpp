...
void layerone_read_block_matrix(
    ...
    for(int n = 0; n < 128 * 128; n++){
        ...
        AX_DDR_bank1 >> mydata;
        ap_uint<32> idata = mydata.data.range(32 - 1, 0);
        float value = *reinterpret_cast<float *>(&idata);
        AX_stream <<  value;
    }
}
...
void layerone_MergeResult(
    float localW[BLOCK_NUM][VDATA_SIZE],
    hls::stream<float> &AX_stream,
    ...
    for(int n = 0; n < 128*128; n++){
        ...
        if(merge == 1){
            ap_axiu<512, 0, 0, 0> mydata1;  AX_Merge_Stream_bank1 >> mydata1;
            ap_axiu<512, 0, 0, 0> mydata2;  AX_Merge_Stream_bank2 >> mydata2;
            ap_axiu<512, 0, 0, 0> mydata3;  AX_Merge_Stream_bank3 >> mydata3;

            for(int k = 0; k < 16; k++){
                ap_uint<32> idata = mydata1.data.range((k + 1)*32 - 1, k*32);
                tmpIn1.data[k] = *reinterpret_cast<float *>(&idata);
            }
            for(int k = 0; k < 16; k++){
                ap_uint<32> idata = mydata2.data.range((k + 1)*32 - 1, k*32);
                tmpIn2.data[k] = *reinterpret_cast<float *>(&idata);
            }
            for(int k = 0; k < 16; k++){
                ap_uint<32> idata = mydata3.data.range((k + 1)*32 - 1, k*32);
                tmpIn3.data[k] = *reinterpret_cast<float *>(&idata);
            }
        }
...
void layerone_ApplyActivationBias(
    ...
    for(int n = 0; n < 128*128; n++){
        ...
        ap_axiu<512, 0, 0, 0> mydata1, mydata2, mydata3;
        ap_uint<512> datapack1, datapack2, datapack3;
        for(int k = 0; k < 16; k++){
            datapack1.range((k + 1)*32 - 1, k*32) = *reinterpret_cast<ap_uint<32> *>(&tmpIn1.data[k]);
            datapack2.range((k + 1)*32 - 1, k*32) = *reinterpret_cast<ap_uint<32> *>(&tmpIn2.data[k]);
            datapack3.range((k + 1)*32 - 1, k*32) = *reinterpret_cast<ap_uint<32> *>(&tmpIn3.data[k]);
        }
...
void layerone_startExecute(
    ...
    int biasflag,
    int layerSelect,
    float localW[2][BLOCK_NUM][VDATA_SIZE],
    float bias[2][BLOCK_NUM][VDATA_SIZE],
    hls::stream<ap_axiu<512, 0, 0, 0>> &  AX_Merge_Stream_bank1,
    hls::stream<ap_axiu<512, 0, 0, 0>> &  AX_Merge_Stream_bank2,
    hls::stream<ap_axiu<512, 0, 0, 0>> &  AX_Merge_Stream_bank3,
    ...
    static hls::stream<float> AX_stream;
...
void layerone(
    ...
    static float localW[2][BLOCK_NUM][VDATA_SIZE];
    ...
    static float bias[2][BLOCK_NUM][VDATA_SIZE];
