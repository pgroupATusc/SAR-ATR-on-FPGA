...
void loadweightL1(
    ...
    for(int j = 0; j < 4; j++){
        ...
        int index = 64 * 256 * 3 + j;
        v_dt tmpIn = weight_port[index];
        for(int k = 0; k < 16; k++){
            #pragma HLS loop_flatten
            biasl1[j*16 + k] = tmpIn.data[k];
        }
    }
}
...
void loadweightL2(
    ...
    for(int i = 0; i < 64; i++){
        ...
        // for(int j = 0; j < 10; j++){
        //     printf("weightL2[%d]:%.12f", i, weightL2[i].data[j]);
        // }
        // printf("\n");
    }

    biasl2 = weight_portL2[64];
    // for(int i = 0; i < 10; i++){
    //     printf("biasl2[%d]:%.12f ", i, biasl2.data[i]);
    // }
    // printf("\n");
}
