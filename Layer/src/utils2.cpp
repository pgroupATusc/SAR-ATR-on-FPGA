            ...
            for(int j = 0; j < VDATA_SIZE; j++){
                if(f == 0){
                    if(j == 0){
                        ...
                        input_map_bank1[i].data[j] = (datatype) fdata;
                    }
                    else{
        ...
        for(int kk = 0; kk < BLOCK_NUM; kk++){  
            for(int j = 0; j < VDATA_SIZE; j++)
            {
                if(i == 0){
                    ...
                    boW_map_self[i*BLOCK_NUM + kk].data[j] = (datatype) fdata;
        ...
        for(int kk = 0; kk < BLOCK_NUM; kk++){  
            for(int j = 0; j < VDATA_SIZE; j++)
            {
                if(i == 0){  // load weights
                    ...
                    boW_neighbor_map[i*BLOCK_NUM + kk].data[j] = (datatype) fdata;
                }
...
void layerone_load_gconv1(
    ...
    for(int kk = 0; kk < BLOCK_NUM; kk++){  
        for(int j = 0; j < VDATA_SIZE; j++)
        {
            ...
            boW_map_self[kk].data[j] = (datatype) fdata;
            // printf("boW_map_self[%d][%d]: %f\n", kk, j, (float) boW_map_self[kk].data[j]);
        ...
        for(int kk = 0; kk < BLOCK_NUM; kk++){  
            for(int j = 0; j < VDATA_SIZE; j++)
            {
                if(i == 0){
                    fin_weight.read(reinterpret_cast<char*>(&fdata), sizeof(float));
                    boW_neighbor_map[i*BLOCK_NUM + kk].data[j] = (datatype) fdata;
                    // printf("boW_neighbor_map[%d][%d]: %.12f\n", i*BLOCK_NUM + kk, j, (float) boW_neighbor_map[i*BLOCK_NUM + kk].data[j]);
                }
                else if(i == 1){
...
void load_gconv2(
    ...
    for(int i = 0; i < BLOCK_NUM; i++){
        for(int ii = 0; ii < VDATA_SIZE; ii++){
            for(int kk = 0; kk < BLOCK_NUM; kk++){  
                ...
                {
                    fin_weight.read(reinterpret_cast<char*>(&fdata), sizeof(float));
                    boW_gconv2_map_self[(kk*VDATA_SIZE + j) * BLOCK_NUM + i].data[ii] = (datatype) fdata;
                    // printf("boW_gconv2_map_self %f\n", (float) fdata);
                    // if( fdata > 65500.0 || fdata < -65500.0){
                    //     printf("%.12f overflow happens:\n", fdata);
                    // }
                }
            }
        }
    ...
    for(int i = 0; i < BLOCK_NUM; i++){
        for(int ii = 0; ii < VDATA_SIZE; ii++){
            for(int kk = 0; kk < BLOCK_NUM; kk++){  
                for(int j = 0; j < VDATA_SIZE; j++){
                    fin_weight_neighbor.read(reinterpret_cast<char*>(&fdata), sizeof(float));
                    boW_gconv2_neighbor_map[(kk*VDATA_SIZE + j) * BLOCK_NUM + i].data[ii] = (datatype) fdata;
                    // if(fdata > 65500.0 || fdata < 65500.0){
                    //     printf("%.12f overflow happens: %.12f, %.12f!\n", fdata);
                    // }
                }
            }
        }
    ...
    for(int kk = 0; kk < BLOCK_NUM; kk++){  
        for(int j = 0; j < VDATA_SIZE; j++)
        {
            ...
            fin_bias_neighbor.read(reinterpret_cast<char*>(&fdata), sizeof(float));
            boW_gconv2_neighbor_map[i*BLOCK_NUM + kk].data[j] = (datatype) fdata;
            // if(fdata > 65500.0 || fdata < 65500.0){
            //     printf("%.12f overflow happens: %.12f, %.12f!\n", fdata);
            // }
        }
...
void load_gconv3(
    ...
    for(int i = 0; i < BLOCK_NUM; i++){
        for(int ii = 0; ii < VDATA_SIZE; ii++){
            for(int kk = 0; kk < BLOCK_NUM; kk++){  
                ...
                {
                    fin_weight.read(reinterpret_cast<char*>(&fdata), sizeof(float));
                    boW_gconv3_map_self[(kk*VDATA_SIZE + j) * BLOCK_NUM + i].data[ii] =  (datatype) fdata;
                }
            }
        }
    }
    ...
    for(int i = 0; i < BLOCK_NUM; i++){
        for(int ii = 0; ii < VDATA_SIZE; ii++){
            for(int kk = 0; kk < BLOCK_NUM; kk++){  
                for(int j = 0; j < VDATA_SIZE; j++){
                    fin_weight_neighbor.read(reinterpret_cast<char*>(&fdata), sizeof(float));
                    boW_gconv3_neighbor_map[(kk*VDATA_SIZE + j) * BLOCK_NUM + i].data[ii] = (datatype) fdata;
                }
            }
        }
    }
    ...
    for(int kk = 0; kk < BLOCK_NUM; kk++){  
        for(int j = 0; j < VDATA_SIZE; j++)
        {
            int i = BLOCK_NUM * VDATA_SIZE;
            fin_bias_neighbor.read(reinterpret_cast<char*>(&fdata), sizeof(float));
            boW_gconv3_neighbor_map[i*BLOCK_NUM + kk].data[j] = (datatype) fdata;
...
void load_gconv4(
    ...
    for(int i = 0; i < BLOCK_NUM; i++){
        for(int ii = 0; ii < VDATA_SIZE; ii++){
            for(int kk = 0; kk < BLOCK_NUM; kk++){  
                ...
                {
                    fin_weight.read(reinterpret_cast<char*>(&fdata), sizeof(float));
                    boW_gconv4_map_self[(kk*VDATA_SIZE + j) * BLOCK_NUM + i].data[ii] = (datatype) fdata;
                }
            }
        }
    }
    ...
    for(int i = 0; i < BLOCK_NUM; i++){
        for(int ii = 0; ii < VDATA_SIZE; ii++){
            for(int kk = 0; kk < BLOCK_NUM; kk++){  
                for(int j = 0; j < VDATA_SIZE; j++){
                    fin_weight_neighbor.read(reinterpret_cast<char*>(&fdata), sizeof(float));
                    boW_gconv4_neighbor_map[(kk*VDATA_SIZE + j) * BLOCK_NUM + i].data[ii] =  (datatype) fdata;
                }
            }
        }
    }
    ...
    for(int kk = 0; kk < BLOCK_NUM; kk++){  
        for(int j = 0; j < VDATA_SIZE; j++)
        {
            int i = BLOCK_NUM * VDATA_SIZE;
            fin_bias_neighbor.read(reinterpret_cast<char*>(&fdata), sizeof(float));
            boW_gconv4_neighbor_map[i*BLOCK_NUM + kk].data[j] = (datatype) fdata;
        ...
        for(int kk = 0; kk < 3; kk++){
            for(int j = 0; j < VDATA_SIZE; j++){
                ...
                if(kk == 0 && fdata != boC_pooling1_bank1_map[i].data[j]) 
                    printf("pooling 1: (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_pooling1_bank1_map[i].data[j]);
                if(kk == 1 && fdata != boC_pooling1_bank2_map[i].data[j]) 
                    printf("pooling 1: (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_pooling1_bank2_map[i].data[j]);
                if(kk == 2 && fdata != boC_pooling1_bank3_map[i].data[j]) 
                    printf("pooling 1: (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_pooling1_bank3_map[i].data[j]);
        ...
        for(int kk = 0; kk < 3; kk++){
            for(int j = 0; j < VDATA_SIZE; j++){
                ...
                if(kk == 0 && fdata != boC_pooling2_bank1_map[i].data[j]) 
                    printf("pooling 2: (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_pooling2_bank1_map[i].data[j]);
                if(kk == 1 && fdata != boC_pooling2_bank2_map[i].data[j]) 
                    printf("pooling 2: (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_pooling2_bank2_map[i].data[j]);
                if(kk == 2 && fdata != boC_pooling2_bank3_map[i].data[j]) 
                    printf("pooling 2: (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_pooling2_bank3_map[i].data[j]);
        ...
        for(int kk = 0; kk < 3; kk++){
            for(int j = 0; j < VDATA_SIZE; j++){
                ...
                if(kk == 0 && fdata != boC_pooling3_bank1_map[i].data[j]) 
                    printf("pooling 3: (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_pooling3_bank1_map[i].data[j]);
                if(kk == 1 && fdata != boC_pooling3_bank2_map[i].data[j]) 
                    printf("pooling 3: (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_pooling3_bank2_map[i].data[j]);
                if(kk == 2 && fdata != boC_pooling3_bank3_map[i].data[j]) 
                    printf("pooling 3: (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float )boC_pooling3_bank3_map[i].data[j]);
        ...
        for(int kk = 0; kk < BLOCK_NUM; kk++){
            for(int j = 0; j < VDATA_SIZE; j++){
                ...
                if(kk == 0 && fdata != boC_gconv2_map_bank1[i].data[j]) 
                    printf("After_gconv2 (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_gconv2_map_bank1[i].data[j]);
                if(kk == 1 && fdata != boC_gconv2_map_bank2[i].data[j]) 
                    printf("After_gconv2 (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_gconv2_map_bank2[i].data[j]);
                if(kk == 2 && fdata != boC_gconv2_map_bank3[i].data[j]) 
                    printf("After_gconv2 (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_gconv2_map_bank3[i].data[j]);
        ...
        for(int kk = 0; kk < BLOCK_NUM; kk++){
            for(int j = 0; j < VDATA_SIZE; j++){
                ...
                if(kk == 0 && fdata != boC_gconv3_map_bank1[i].data[j]) 
                    printf("After_gconv3 (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_gconv3_map_bank1[i].data[j]);
                if(kk == 1 && fdata != boC_gconv3_map_bank2[i].data[j]) 
                    printf("After_gconv3 (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_gconv3_map_bank2[i].data[j]);
                if(kk == 2 && fdata != boC_gconv3_map_bank3[i].data[j]) 
                    printf("After_gconv3 (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_gconv3_map_bank3[i].data[j]);
        ...
        for(int kk = 0; kk < 3; kk++){
            for(int j = 0; j < 16; j++){
                ...
                if(kk == 0 && fdata != boC_gconv4_map_bank1[i].data[j]) 
                    printf("After_gconv4 (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_gconv4_map_bank1[i].data[j]);
                if(kk == 1 && fdata != boC_gconv4_map_bank2[i].data[j]) 
                    printf("After_gconv4 (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_gconv4_map_bank2[i].data[j]);
                if(kk == 2 && fdata != boC_gconv4_map_bank3[i].data[j]) 
                    printf("After_gconv4 (%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, (float) boC_gconv4_map_bank3[i].data[j]);
...
void allocateMLP(
        ...
        for(int j = 0; j < 256; j++){
            for(int k = 0; k < 3; k++){
                ...
                for(int kk = 0; kk < 16; kk++){
                    ...
                    weight_l1_map[index].data[kk] = (datatype) fdata;
                }
            }
        }
    }
    ...
    for(int i = 0; i < 4; i++){
        ...
        for(int j = 0; j < 16; j++){
            ...
            weight_l1_map[index].data[j] = (datatype) fdata;
            // printf("fc1.bias: %.16f\n", fdata);
        }
    }
    ...
    for(int i = 0 ; i < 10; i++){
        for(int j = 0; j < 64; j++){
            ...
            weight_l2_map[j].data[i] = (datatype) fdata;
        }
    }
    ...
    for(int i = 0; i < 10; i++){
        ...
        weight_l2_map[64].data[i] = (datatype) fdata;
    }
