...
# data = run("cat cpupower.text",capture_output=True,shell=True)
...
for epoch in range(100):
    ...
    for i in range(2700):
        ...
        if i % 30 == 0:
            timestr = " %.0f " % (30.0/sumtime)
            Latency = sumtime/30.0 * 1000
            sumtime = 0
        ...
        iimage = cv2.putText(iimage, 'Epoch %d, image %d' % (epoch, i), (400, 50), font, 
                    fontScale, color2, thickness, cv2.LINE_AA)
        iimage = cv2.putText(iimage, 'Latency %.1f ms' % Latency, (400, 100), font, 
                    fontScale, color2, thickness, cv2.LINE_AA)
