...
void loadMerge(
    const v_dt * AX_Merge,  
    hls::stream<v_dt> & AX_Merge_DDR,
    int block_row_num,
    int merge
){
    #pragma HLS FUNCTION_INSTANTIATE variable=AX_Merge
    #pragma HLS FUNCTION_INSTANTIATE variable=AX_Merge_DDR
    if(merge == 1){
        for(int n = 0; n < block_row_num; n++){
            for(int ii = 0; ii < VDATA_SIZE; ii++){
                #pragma HLS loop_flatten
...
void MergeResult(
    hls::stream<v_dt> & AX_Merge_DDR_bank1,
    hls::stream<v_dt> & AX_Merge_DDR_bank2,
    hls::stream<v_dt> & AX_Merge_DDR_bank3,
    hls::stream<v_dt> & C_stream,  
        ...
        for(int n = 0; n < block_row_num; n++){
            for(int ii = 0; ii < VDATA_SIZE; ii++){
                for(int j = 0; j < BLOCK_NUM; j++){
                    ...
                    if (j == 0) tmpIn1 = AX_Merge_DDR_bank1.read();
                    else if (j == 1) tmpIn1 = AX_Merge_DDR_bank2.read();
                    else tmpIn1 = AX_Merge_DDR_bank3.read();
...
)
{
    ...
    // #pragma HLS INTERFACE m_axi port = AX_Merge_bank1 offset = slave bundle = gmem7
    // #pragma HLS INTERFACE m_axi port = AX_Merge_bank2 offset = slave bundle = gmem8
    // #pragma HLS INTERFACE m_axi port = AX_Merge_bank3 offset = slave bundle = gmem9
    ...
    static hls::stream<v_dt> AX_Merge_DDR[3];
    #pragma HLS STREAM variable=AX_Merge_DDR depth=64 type=fifo
    #pragma HLS ARRAY_PARTITION variable=AX_Merge_DDR dim=1 complete
    ...
    loadMerge(AX_Merge_bank1, AX_Merge_DDR[0], block_row_num, merge);
    loadMerge(AX_Merge_bank2, AX_Merge_DDR[1], block_row_num, merge);
    loadMerge(AX_Merge_bank3, AX_Merge_DDR[2], block_row_num, merge);

    MergeResult(AX_Merge_DDR[0], AX_Merge_DDR[1], AX_Merge_DDR[2], C_stream, F_stream, block_row_num, merge);
