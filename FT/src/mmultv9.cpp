    ...
    for(int n = 0; n < block_row_num; n++){
        for(int ii = 0; ii < VDATA_SIZE; ii++){
            for(int j = 0; j < BLOCK_NUM; j++){
                ...
                if(j == 0) R_stream_bank1 << mydata;
                else if(j == 1) R_stream_bank2 << mydata;
                else R_stream_bank3 << mydata;
            }
        }
    }
...
// ){
//     #pragma HLS FUNCTION_INSTANTIATE variable=localW
...
//     int biasflag
// ){
//     #pragma HLS FUNCTION_INSTANTIATE variable=bias
...
//             if(biasflag == 1) bias[i][ii] = tmpIn.data[ii];
//             else bias[i][ii] = 0;
//             // bias[i][ii] = 0;
//             // printf("%.0f ", localW[i][j][ii][jj]);
//         }  
...
)
{
    ...
    ApplyActivationBias(F_stream, R_stream1, R_stream2, R_stream3, block_row_num, bias, ReLu, biasflag);
}
}
