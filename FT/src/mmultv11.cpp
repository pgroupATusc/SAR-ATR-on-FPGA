...
void startExecute(
    ...
    int mode,
    int layerSelect,
    float localW[8][BLOCK_NUM][BLOCK_NUM][VDATA_SIZE][VDATA_SIZE],
    ...
    compute(AX_stream, C_stream, localW[layerSelect], block_row_num/6);
...
)
{
    ...
    static float localW[8][BLOCK_NUM][BLOCK_NUM][VDATA_SIZE][VDATA_SIZE];
    ...
    #pragma HLS ARRAY_PARTITION variable=localW dim=5 complete
    ...
    if(mode == 1){
        loadweight(W, localW[layerSelect], bias[layerSelect], biasflag);
    }
    else if(mode == 2){
        startExecute(
