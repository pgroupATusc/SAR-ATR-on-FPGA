...
void read_AX_fromDDR_bankDouble(
    const v_dt *AX, 
    int size,
    int mode,
    hls::stream<ap_axiu<512, 0, 0, 0>> & AX_DDR,
    hls::stream<ap_axiu<512, 0, 0, 0>> & tolayerone
){
    #pragma HLS INTERFACE mode=axis register both port=AX_DDR
    #pragma HLS INTERFACE mode=axis register both port=tolayerone
    ...
    for(int n = 0; n < size; n ++){
        ...
        if(mode == 1) AX_DDR << mydata;
        else if(mode == 2) tolayerone << mydata;
    }
