...
void read_block_matrix(
    ...
    hls::stream<ap_axiu<256, 0, 0, 0>> & AX_DDR_bank2, 
    hls::stream<ap_axiu<256, 0, 0, 0>> & AX_DDR_bank3, 
            ...
            for(int ii = 0; ii < VDATA_SIZE; ii++){ // load data from DDR/HBM
                for(int k = 0; k < 3; k++){
                    ...
                    for(int jj = 0; jj < 16; jj++){
                        ap_uint<16> idata = mydata.data.range((jj + 1)*16 - 1, jj*16);
...
void ApplyActivationBias(
    ...
    hls::stream<ap_axiu<256, 0, 0, 0>> &  To_mlp1,
    hls::stream<ap_axiu<256, 0, 0, 0>> &  To_mlp2,
    hls::stream<ap_axiu<256, 0, 0, 0>> &  To_mlp3,
    int block_row_num,
...
void startExecute(
    ...
    hls::stream<ap_axiu<256, 0, 0, 0>> &  AX_DDR_stream2,
    hls::stream<ap_axiu<256, 0, 0, 0>> &  AX_DDR_stream3,
