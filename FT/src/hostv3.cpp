...
int main(int argc, char** argv) {
    ...
    // printf("initailze the data in boW_map\n");
    // // generate random numbers for boW_map
    // for(int i = 0; i < BLOCK_NUM * (BLOCK_NUM * VDATA_SIZE + 1) ; i++){
    //     for(int j = 0; j < VDATA_SIZE; j++){
    //         boW_map[i].data[j] = rand() % 10;
    //         // printf("%.0f ", boW_map[i].data[j]);
    //     }
    //     // if( i % BLOCK_NUM == BLOCK_NUM - 1) printf("\n");
    // }
    ...
    for(int i = 0; i < BM * VDATA_SIZE;  i++){
        for(int j = 0; j < BLOCK_NUM; j++){
            for(int jj = 0; jj < VDATA_SIZE; jj++){
                boC_golden[i*BLOCK_NUM + j].data[jj] = boC_golden[i*BLOCK_NUM + j].data[jj] + boW_map[biasindex + j].data[jj];
            }
        }
    }
    ...
    std::ifstream fin_selfT("../data/neighborT.bin", std::ios::binary);
