...
int main(int argc, char** argv) {
    ...
    boAX_Merge.sync(XCL_BO_SYNC_BO_TO_DEVICE);
    ...
    auto run = krnl(boAX, boAX_Merge, boW, boC, BM, 1, 1); //DATA_SIZE=size
    run.wait();
        ...
        for(int kk = 0; kk < BLOCK_NUM; kk++){
            for(int j = 0; j < VDATA_SIZE; j++){
                if(i < 16384){
                    ...
                    printf("(%d, %d) golden: %f, output: %f\n", i, kk * VDATA_SIZE + j, fdata, boC_map[i * BLOCK_NUM + kk].data[j]);
                }
            }
        }
    }
