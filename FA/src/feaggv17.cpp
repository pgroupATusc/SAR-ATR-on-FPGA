...
void Compute(
    ...
    for(int i = 0; i < num_edge_block; i++){
        ...
        int dst = iedge.dst;
        datatype value = (datatype) iedge.value;
        int flag = iedge.flag;
...
void working(
    ...
    hls::stream<edge> A_stream[NUM_FEA_BANK];
    #pragma HLS STREAM variable=A_stream depth=16 type=fifo
    #pragma HLS ARRAY_PARTITION variable=A_stream dim=1 complete


    loadA_ind(A, A_stream, num_edge_block);
    
    Compute(InVFeature[0], A_stream[0], OutVFeature[0], num_edge_block, imagedimension, maxpooling);
    Compute(InVFeature[1], A_stream[1], OutVFeature[1], num_edge_block, imagedimension, maxpooling);
    Compute(InVFeature[2], A_stream[2], OutVFeature[2], num_edge_block, imagedimension, maxpooling);
    Compute(InVFeature[3], A_stream[3], OutVFeature[3], num_edge_block, imagedimension, maxpooling);
    // for(int i = 0; i < NUM_FEA_BANK; i++){
    //     #pragma HLS unroll
...
void loadMerge(
    ...
    hls::stream<ap_axiu<sizeof(datatype) *8 * 16, 0, 0, 0>> & AX_Merge_DDR,
    int block_row_num,
    int merge
){
...
void executeLoadMerge(
    ...
    #pragma HLS dataflow
    loadMerge(AX_Merge_bank1, AX_Merge_Stream_bank1, block_row_num, merge);
    loadMerge(AX_Merge_bank2, AX_Merge_Stream_bank2, block_row_num, merge);
    loadMerge(AX_Merge_bank3, AX_Merge_Stream_bank3, block_row_num, merge);
}
...
void feagg_top(
    ...
    if(mode == 1){
        ...
        executeLoadMerge(
            ...
            block_row_num, merge, AX_Merge_Stream_bank1, AX_Merge_Stream_bank2, AX_Merge_Stream_bank3
        );
    }
