...
int main(int argc, char** argv) {
    ...
    auto boA_p1 = xrt::bo(device, size_t(sizeof(v_edge) * num_edge_block / 2), krnl.group_id(1));
    auto boA_p2 = xrt::bo(device, size_t(sizeof(v_edge) * num_edge_block / 2), krnl.group_id(1));
    ...
    auto run_p1 = krnl(boX, boA_p1, boAX, num_vertex_bank, num_edge_block/2, f_block_len_custom, imagescale, 1, 0, 0, 0); 
    // run_p1.wait();
    // std::cout << "Execution of the kernel_p2\n";
    auto run_p2 = krnl(boX, boA_p2, boAX, num_vertex_bank, num_edge_block/2, f_block_len_custom, imagescale, 0, 1, num_vertex_bank*4, num_vertex_bank*4); 
    run_p1.wait();
    run_p2.wait();
    ...
    for(int i = 0; i < num_vertex * f_block_len_custom; i++){
        for(int j = 0; j < VDATA_SIZE; j++){
            if(boAX_golden[i].data[j] != boAX_map[i].data[j]) counter++;
            // if( i < 16)
