...
int main(int argc, char** argv) {
    ...
    auto load_2 = xrt::kernel(device, uuid, "read_AX_fromDDR_bank:{read_AX_fromDDR_bank_2}", xrt::kernel::cu_access_mode::exclusive);
    auto load_3 = xrt::kernel(device, uuid, "read_AX_fromDDR_bank:{read_AX_fromDDR_bank_3}", xrt::kernel::cu_access_mode::exclusive);
    ...
    auto boAX_bank1 = xrt::bo(device, size_t(sizeof(float) * BM  * VDATA_SIZE * VDATA_SIZE), load_1.group_id(0)); //Match kernel arguments to RTL kernel
    auto boAX_bank2 = xrt::bo(device, size_t(sizeof(float) * BM  * VDATA_SIZE * VDATA_SIZE), load_2.group_id(0)); //Match kernel arguments to RTL kernel
    auto boAX_bank3 = xrt::bo(device, size_t(sizeof(float) * BM  * VDATA_SIZE * VDATA_SIZE), load_3.group_id(0)); //Match kernel arguments to RTL kernel
    ...
    auto boAX_Merge_bank1 = xrt::bo(device, size_t(sizeof(float) * BM * VDATA_SIZE * VDATA_SIZE), load_4.group_id(0));
    auto boAX_Merge_bank2 = xrt::bo(device, size_t(sizeof(float) * BM * VDATA_SIZE * VDATA_SIZE), load_5.group_id(0));
    auto boAX_Merge_bank3 = xrt::bo(device, size_t(sizeof(float) * BM * VDATA_SIZE * VDATA_SIZE), load_6.group_id(0));
    
    std::cout << "Allocate boW at bank"  << "\n";
    auto boW = xrt::bo(device, size_t(sizeof(float) * (BLOCK_NUM * VDATA_SIZE  + 1)* BLOCK_NUM  * VDATA_SIZE), krnl.group_id(0));

    std::cout << "Allocate boC at bank"  << "\n";
    auto boC_bank1 = xrt::bo(device, size_t(sizeof(float) * BM  * VDATA_SIZE * VDATA_SIZE), store_1.group_id(0));
    auto boC_bank2 = xrt::bo(device, size_t(sizeof(float) * BM  * VDATA_SIZE * VDATA_SIZE), store_2.group_id(0));
    auto boC_bank3 = xrt::bo(device, size_t(sizeof(float) * BM  * VDATA_SIZE * VDATA_SIZE), store_3.group_id(0));
    ...
    auto run = krnl(
        ...
        1,
        1); //DATA_SIZE=size
    auto run_load_1 = load_1(boAX_bank1, BM);
    auto run_load_2 = load_2(boAX_bank2, BM);
    auto run_load_3 = load_3(boAX_bank3, BM);
    auto run_load_4 = load_4(boAX_Merge_bank1, BM);
    auto run_load_5 = load_5(boAX_Merge_bank2, BM);
    auto run_load_6 = load_6(boAX_Merge_bank3, BM);
    auto run_store_1 = store_1(boC_bank1, BM);
    auto run_store_2 = store_2(boC_bank2, BM);
    auto run_store_3 = store_3(boC_bank3, BM);
