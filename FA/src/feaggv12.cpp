...
void loadX(
    v_float * X, 
    v_float InVFeature[NUM_FEA_BANK][NUM_VER_PER_BANK], 
    int num_vertex_bank,
    int fid,
    int imagedimension,
    int ignorehead,
...
void InitializeAX(
    v_float OutVFeature[NUM_FEA_BANK][NUM_VER_PER_BANK], 
    int fid,
    int num_vertex_bank
){
    #pragma HLS FUNCTION_INSTANTIATE variable=fid
...
void Compute(
    v_float InVFeature[NUM_VER_PER_BANK], 
    hls::stream<edge> & A_stream, 
    v_float OutVFeature[NUM_VER_PER_BANK], 
    int num_edge_block,
    int imagedimension,
    int maxpooling
){
    ...
    for(int i = 0; i < num_edge_block; i++){
        ...
        v_float mysrcvf = InVFeature[src];
        v_float mydstvf = OutVFeature[dst];
...
void feagg1(
    ...
    v_float InVFeature1[NUM_FEA_BANK][NUM_VER_PER_BANK];
    ...
    v_float OutVFeature1[NUM_FEA_BANK][NUM_VER_PER_BANK];
    ...
    v_float * newX = X + xoffset;
    v_float * newAX = AX + axoffset;
...
void feagg2(
    ...
    // define the buffer to store the vertex features
    v_float InVFeature2[NUM_FEA_BANK][NUM_VER_PER_BANK];
    ...
    // define the buffer to store the results
    v_float OutVFeature2[NUM_FEA_BANK][NUM_VER_PER_BANK];
...
void feagg3(
    ...
    // define the buffer to store the vertex features
    v_float InVFeature3[NUM_FEA_BANK][NUM_VER_PER_BANK];
    ...
    // define the buffer to store the results
    v_float OutVFeature3[NUM_FEA_BANK][NUM_VER_PER_BANK];
...
void loadMerge(
    const v_float * AX_Merge,  
    hls::stream<ap_axiu<512, 0, 0, 0>> & AX_Merge_DDR,
    hls::stream<ap_axiu<512, 0, 0, 0>> & AX_Merge_layerone,
    int block_row_num,
    int merge,
    int mode
){
    ...
    if(merge == 1){
        for(int n = 0; n < block_row_num; n++){
            for(int ii = 0; ii < VDATA_SIZE; ii++){
                ...
                if(mode == 2) AX_Merge_DDR << mydata;
                else if (mode == 3 && (n * VDATA_SIZE + ii) < 128*128) AX_Merge_layerone << mydata;
            }
        }
    }
...
void executeLoadMerge(
    const v_float * AX_Merge_bank1,       // Read-Only AX_Merge
    const v_float * AX_Merge_bank2,       // Read-Only AX_Merge
    const v_float * AX_Merge_bank3,       // Read-Only AX_Merge
    int block_row_num,
    int merge,
    int mode,
    ...
    hls::stream<ap_axiu<512, 0, 0, 0>> &  AX_Merge_layerone_bank1,
    hls::stream<ap_axiu<512, 0, 0, 0>> &  AX_Merge_layerone_bank2,
    hls::stream<ap_axiu<512, 0, 0, 0>> &  AX_Merge_layerone_bank3
){
    #pragma HLS dataflow
    loadMerge(AX_Merge_bank1, AX_Merge_Stream_bank1, AX_Merge_layerone_bank1, block_row_num, merge, mode);
    loadMerge(AX_Merge_bank2, AX_Merge_Stream_bank2, AX_Merge_layerone_bank2, block_row_num, merge, mode);
    loadMerge(AX_Merge_bank3, AX_Merge_Stream_bank3, AX_Merge_layerone_bank3, block_row_num, merge, mode);
}
...
void feagg_top(
    ...
    #pragma HLS INTERFACE mode=axis register both port=AX_Merge_layerone_bank1
    #pragma HLS INTERFACE mode=axis register both port=AX_Merge_layerone_bank2
    #pragma HLS INTERFACE mode=axis register both port=AX_Merge_layerone_bank3
    ...
    #pragma HLS INTERFACE m_axi port = AX_Merge_bank1 offset = slave bundle = gmem0
    #pragma HLS INTERFACE m_axi port = AX_Merge_bank2 offset = slave bundle = gmem1
    #pragma HLS INTERFACE m_axi port = AX_Merge_bank3 offset = slave bundle = gmem2
    ...
    if(mode == 1){
        executeAgg(
            X_bank1, X_bank2, X_bank3, A, AX_bank1, AX_bank2, AX_bank3,
            num_vertex_bank, num_edge_block, feature_block_len, imagedimension,
            ignorehead, ignoretail, xoffset, axoffset, num_vertex_bank_store, maxpooling
        );
    }
    else if(mode > 1){
        executeLoadMerge(
            AX_Merge_bank1, AX_Merge_bank2, AX_Merge_bank3, 
            block_row_num, merge, mode, AX_Merge_Stream_bank1, AX_Merge_Stream_bank2, AX_Merge_Stream_bank3,
            AX_Merge_layerone_bank1, AX_Merge_layerone_bank2, AX_Merge_layerone_bank3
        );
    }
