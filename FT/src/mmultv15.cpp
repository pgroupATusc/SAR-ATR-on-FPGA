            ...
            for(int ii = 0; ii < VDATA_SIZE; ii++){ // load data from DDR/HBM
                for(int k = 0; k < 3; k++){
                    ...
                    for(int jj = 0; jj < 16; jj++){
                        ...
                        if(layerSelect >= 2) {localAXblock[nn][k][ii][jj] = *reinterpret_cast<float *>(&idata);}         // layer 3, 4, 5, 6, 7, 8
                        else if(layerSelect < 2 && jj == 0 && k == 0) {RM_stream << *reinterpret_cast<float *>(&idata);};    // layer 1 and 2
                    }
                }
            }
        }
...
void compute(hls::stream<v_dt> &AX_stream, hls::stream<v_dt> & C_stream, const float localW[BLOCK_NUM][BLOCK_NUM][VDATA_SIZE][VDATA_SIZE], int block_row_num, int layerSelect){
...
void loadweight(
    const v_dt *W, 
    float localW[2][8][BLOCK_NUM][BLOCK_NUM][VDATA_SIZE][VDATA_SIZE],
    float bias[BLOCK_NUM][VDATA_SIZE],
    int biasflag,
    int layerSelect
){
...
void startExecute(
    ...
    int layerSelect,
    float localW[2][8][BLOCK_NUM][BLOCK_NUM][VDATA_SIZE][VDATA_SIZE],
    float bias[8][BLOCK_NUM][VDATA_SIZE],
    hls::stream<ap_axiu<512, 0, 0, 0>> &  AX_Merge_Stream_bank1,
    hls::stream<ap_axiu<512, 0, 0, 0>> &  AX_Merge_Stream_bank2,
    hls::stream<ap_axiu<512, 0, 0, 0>> &  AX_Merge_Stream_bank3,
    ...
    static hls::stream<float> RM_stream;
...
)
{
    ...
    static float localW[2][8][BLOCK_NUM][BLOCK_NUM][VDATA_SIZE][VDATA_SIZE];
    ...
    static float bias[8][BLOCK_NUM][VDATA_SIZE];
